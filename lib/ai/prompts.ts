...
export function buildDeepSeekPaperAnalysisPrompt({
  ...
  return [
    ...
    "cases 生成 1-3 个即可，优先选择最清晰、最容易由原文支持的主线。每个 case 必须包含：case_id, paper_id, case_title, main_claim, difficulty, evidence_required, recommended, estimated_minutes, involved_figures, experiment_types。",
    ...
    "id 必须使用 ai_evidence_001、ai_evidence_002、ai_evidence_003 这类稳定编号。",
    ...
    "本轮优先输出 source_type 为 text 的 evidence；不要依赖 PDF 图表 bbox 或图像识别。",
    "strength 只能是 weak、medium、strong。",
    "found 必须是 false。",
    "bbox 本轮通常填 null；不要为了图表强行编造 bbox。",
    "confidence 使用 0-1 小数。",
    "每个 case 建议 3-5 条 evidence，至少 3 条 text evidence。",
    "text_anchor 必须尽量逐字取自论文原文中的完整句子或短段落，不能改写，不能翻译，不能总结。",
    "text_anchor 应该足够短，方便用户在网页化 RealPaperViewer 中点击对应句子收集证据。",
    "输出尽量短，不要生成长篇报告。",
